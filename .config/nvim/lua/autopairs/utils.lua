...
M.is_in_quotes = function(line, pos, quote_type)
  ...
  while cIndex < string.len(line) and cIndex < pos do
    ...
    if result == true and char == last_char and line:sub(cIndex - 1, cIndex - 1) ~= "\\" then
      result = false
      last_char = quote_type or ""
    elseif result == false and M.is_quote(char) and (not quote_type or char == quote_type) then
